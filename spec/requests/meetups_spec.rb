...
RSpec.describe 'Meetups', type: :request do
	...
	describe 'POST /spots/:spot_id/meetups' do
		it 'works' do
			sign_in(create(:user))
			post "/spots/#{@spot.id}/meetups", meetup: { title: 'Test meetup title', description: 'Test meetup description', time: DateTime.now }
      ...
      headers = {
        ...
        'CONTENT_TYPE' => 'application/json'
			...
			sign_in(create(:user))
      params = { title: 'Test meetup title', description: 'Test meetup description', time: DateTime.now }
      post("/spots/#{@spot.id}/meetups", params.to_json, headers)
		...
		it 'does not create with invalid params' do
			sign_in(create(:user))
			post "/spots/#{@spot.id}/meetups", meetup: { title: nil, description: 'Test meetup description', time: DateTime.now }
			expect(response).to have_http_status(200)
      ...
      headers = {
        ...
        'CONTENT_TYPE' => 'application/json'
			...
			sign_in(create(:user))
      params = { title: nil, description: 'Test meetup description', time: DateTime.now }
      post("/spots/#{@spot.id}/meetups", params.to_json, headers)
